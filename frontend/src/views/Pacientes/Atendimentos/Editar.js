...
class Editar extends Component {
    ...
    render() {
        ...
        const listarLeito = this.state.leito.map(leito => (
            <option key={this.state.leito.id} value={this.state.leito.nome}>{this.state.leito.nome}</option>
        ));

        const listarPacientes = this.state.paciente.map(paciente => (
            <option key={this.state.pacientes.id} value={this.state.paciente.nome}>{this.state.leito.nome}</option>
        ));

        return (
            <Container fluid className="main-content-container px-4 pb-4">
                <Row noGutters className="page-header py-4">
                    <PageTitle sm="4" title="Editar" subtitle="Atendimento" className="text-sm-left" />
                </Row>      
